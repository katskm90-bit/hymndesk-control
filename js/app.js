...
(function () {
  ...
  const MODULES = [
    ...
    { id: 'calendar',     title: 'Calendar',                    icon: 'calendar', roles: null },
  ...
  const ICONS = {
    ...
    calendar:  '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4"/><path d="M8 2v4"/><path d="M3 10h18"/></svg>',
  ...
  function renderModulePlaceholder(mod) {
    ...
    if (mod.id === 'calendar' && window.HD_Calendar) {
      main.innerHTML = '';
      window.HD_Calendar.render(main, { supabase: state.supabase });
      return;
    }
